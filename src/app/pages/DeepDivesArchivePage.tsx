...
import { Calendar, Mail, MapPin, ChevronRight, FileText, Download, TrendingUp, ChevronLeft, BookOpen } from "lucide-react";
...
export function DeepDivesArchivePage() {
  ...
  // Get unique topics from deep dive articles
  const allTopics = Array.from(new Set(deepDiveArticles.flatMap(article => article.topics || [])));
  const topicOptions = ["All", ...allTopics];

  // Filter articles by topic
  const filteredArticles = selectedTopic === "All" 
    ? deepDiveArticles 
    : deepDiveArticles.filter(article => article.topics?.includes(selectedTopic));
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-[1440px] mx-auto px-8 py-3">
          <div className="flex items-center gap-2 text-[13px] text-[var(--slate-medium)]">
            ...
            <span>/</span>
            <span className="text-[var(--navy-deep)]">Deep Dives</span>
          </div>
        </div>
      </div>
              ...
              <section className="mb-12">
                ...
                <div className="space-y-6">
                  {paginatedArticles.map((article) => (
                    <Link
                      key={article.id}
                      to={`/deep-dive/${article.id}`}
                      className="bg-white border border-gray-200 overflow-hidden group hover:shadow-xl hover:border-[var(--electric-blue)]/30 transition-all block h-[240px]"
                    >
                      <div className="grid grid-cols-3 gap-6 h-full">
                        ...
                        <div className="col-span-2 p-5 flex flex-col h-full">
                          <div className="mb-3">
                            <span className={`${article.categoryColor} text-white px-2.5 py-1 text-[10px] tracking-wide uppercase`}>
                              {article.category}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-4 text-[13px] text-[var(--slate-medium)] mt-auto">
                            <span>{article.lastUpdated ? `Last updated ${article.lastUpdated}` : article.publishDate}</span>
                          </div>
                        </div>
                      </div>
                    </Link>
